...
r=$(run_iez "$IEZ" ui swipe --from "330,250" --to "60,250")
assert_ok "$r" "Attempt carousel move to another habit"
sleep 1.5
...
if [ "$selected_id" = "$initial_id" ]; then
  info "Using initially visible habit; carousel did not move to a different card"
else
